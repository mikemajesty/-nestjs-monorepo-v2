...
export class UserController {
  constructor(
    ...
    private readonly userSearch: IUserSearchAdapter
  ) {}
  ...
  @Get('search')
  @Version('1')
  async getUserByEmailWithRelations(@Req() { query }: ApiRequest): Promise<UserEntity> {
    const user = await this.userSearch.execute(query);
    return user;
  }
